...
def _write_adapter() -> None:
    lines = [
        ...
        "",
        "",
        "def _live_frame_payload():",
        "    if _system is not None and hasattr(_system, \"latest_frame_payload\"):",
        "        return _system.latest_frame_payload()",
        "    return {",
        "        \"status\": \"unavailable\",",
        "        \"image\": None,",
        ...
        "        live_frame=_live_frame_payload(),",
        "    )",
        "",
        "",
        "def _api_frame():",
        "    return _live_frame_payload()",
        "",
        "",
        "def _api_stop():",
        ...
        "",
        "",
        "def _api_manual_hold():",
        "    if _system is None or not getattr(_system, \"_connected\", False):",
        "        return {\"error\": \"Hardware not connected\"}",
        "    try:",
        "        active = _system.bridge_rpc.manual_hold()",
        "        return {\"ok\": True, \"active\": active}",
        "    except Exception as exc:",
        '        _logger.warning("manual hold refresh failed: %s", exc)',
        "        return {\"error\": str(exc)}",
        "",
        "",
        ...
        "    except Exception as exc:",
        "        stop_error = exc",
        '        _logger.warning("emergency stop motor stop failed: %s", exc)',
        "    try:",
        "        _system.bridge_rpc.safe_reset()",
        "    except Exception as exc:",
        "        reset_error = exc",
        '        _logger.warning("emergency stop safe_reset failed: %s", exc)',
        "    if stop_error is None and reset_error is None:",
        "        return {\"ok\": True}",
        "    if stop_error is not None and reset_error is not None:",
        "        return {\"error\": f\"motor stop failed: {stop_error}; safe_reset failed: {reset_error}\"}",
        "    if stop_error is not None:",
        "        return {\"error\": f\"motor stop failed; safe_reset succeeded: {stop_error}\"}",
        "    return {\"error\": f\"motor stop succeeded; safe_reset failed: {reset_error}\"}",
        "",
        "",
        "def _api_test_led():",
        "    if _system is None or not getattr(_system, \"_connected\", False):",
        ...
        "        return {\"error\": \"Hardware not connected\"}",
        "    try:",
        "        _system.bridge_rpc.buzzer_control(\"test\")",
        "        return {\"ok\": True}",
        "    except Exception as exc:",
        "        return {\"error\": str(exc)}",
        "",
        ...
        '_ui.expose_api("GET",  "/api/frame",           _api_frame)',
        ...
        '_ui.expose_api("POST", "/api/control/manual_hold",    _api_manual_hold)',
